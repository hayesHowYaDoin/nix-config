{
  ...
  defaultTheme = import ./default/theme.nix;

  themeName = themeConfig.name or defaultTheme.name;
  isGruvboxMaterial = themeName == "gruvbox-material";
  ...
  isNvfTheme = builtins.elem themeName (import ./nvfThemes.nix);
  nvfThemeConfig =
    if isNvfTheme
    then themeConfig
    else {enable = false;};
